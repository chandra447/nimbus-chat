    ...
    def _get_routing_agent(self):
        if self._routing_agent is None:
            self._routing_agent = create_agent(
                ...
                tools=[],
                system_prompt=(
                    'You are the Nimbus orchestrator router. Your job is to decide which '
                    'specialist agent(s) should handle the user request.\n\n'
                    'You have access to registered specialist agent cards (injected as context). '
                    'Each specialist has skills, tags, and examples that describe what it can do.\n\n'
                    'ROUTING RULES:\n'
                    '1. If a request is about travel (destinations, itineraries, budgets, activities), '
                    'select the travel specialist.\n'
                    '2. If a request is about nutrition, diet, meal planning, macros, or supplements, '
                    ...
                    'all relevant specialists. Do not try to pick just one when multiple are relevant.'
                ),
